...
// TODO: Paste your Agent ID here for the Hackathon Demo so you don't have to type it in UI
const DEMO_AGENT_ID = ""; 
...
const VoiceWidget: React.FC<VoiceWidgetProps> = ({ onUserTranscript, onAgentResponse, onTurnComplete }) => {
  ...
  }, [onUserTranscript, onAgentResponse, onTurnComplete]);
    ...
    try {
      ...
      const conversation = await Conversation.startSession({
        ...
        onModeChange: (mode: { mode: string }) => {
           ...
           // Logic: If the agent STOPS speaking, it's usually the end of the AI turn.
           // However, if the agent STARTS speaking, it implies the USER turn just finished.
           // We'll rely on onMessage for text data, but onModeChange can signal state.
           if (speaking && isSessionActive) {
               // Agent started speaking -> User turn definitely over
           }
           
           if (!speaking && isSessionActive) {
               // Agent finished speaking -> Whole turn cycle likely complete
               onTurnCompleteRef.current();
           }
        },
        onMessage: (props: any) => {
            if (props.source === 'user') {
                // When source is 'user', 11Labs sends the committed transcript of what the user said.
                if (props.text) {
                    onUserTranscriptRef.current(props.text);
                }
            } else if (props.source === 'ai') {
                // When source is 'ai', it streams chunks.
                if (props.text) {
                    onAgentResponseRef.current(props.text);
                }
            }
        }
      } as any);
